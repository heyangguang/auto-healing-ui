...
import { envConfig, FIXED_PAGE_SIZE } from './constants';
...
const HostSelectorModal: React.FC<HostSelectorModalProps> = ({
  ...
  const [selectedMap, setSelectedMap] = useState<
    Map<string, AutoHealing.CMDBItem>
  >(new Map());
  ...
  const loadHosts = useCallback(
    async (page: number, env: string, search: string) => {
      setLoading(true);
      try {
        const params: CMDBItemsParams = {
          status: 'active',
          type: 'server',
          page,
        ...
        if (env !== 'all')
          params.environment = env as AutoHealing.CMDBEnvironment;
        ...
        setPagination((prev) => ({
          ...prev,
          current: page,
          total: getTotalFromListResponse(res),
        }));
      } catch {
        setHosts([]);
      } finally {
  ...
  const treeData = useMemo(() => {
    const nodes = Object.entries(envConfig).map(([key, cfg]) => ({
      ...
      title: (
        <span style={{ fontSize: 13 }}>
          {cfg.label}{' '}
          <span style={{ color: '#999' }}>({envStats[key] || 0})</span>
        </span>
      ),
    ...
    const filtered = treeSearchText
      ? nodes.filter((node) => node._label.includes(treeSearchText))
      : nodes;
    return [
      {
        key: 'all',
        title: (
          <span style={{ fontSize: 13 }}>
            全部主机{' '}
            <span style={{ color: '#999' }}>({envStats.all || 0})</span>
          </span>
        ),
        icon: <GlobalOutlined />,
        children: filtered,
  ...
  const excludedCount = excludeHosts.filter(Boolean).length;
      ...
      footer={
        ...
        >
          <div style={{ fontSize: 14 }}>
            已选择{' '}
            <span
              style={{ color: '#3a84ff', fontWeight: 600, margin: '0 4px' }}
            >
              {selectedIps.length}
            </span>{' '}
            台主机
            {excludedCount > 0 && (
              <span style={{ color: '#999', marginLeft: 8 }}>
                (已排除 {excludedCount} 台模板预设主机)
              </span>
            )}
          </div>
          <Space size={12}>
            <Button onClick={onCancel} style={{ borderRadius: 0 }}>
              取消
            </Button>
            <Button
              type="primary"
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 12px' }}>
              <Tree
                ...
                onSelect={(keys) =>
                  keys[0] && setSelectedEnv(keys[0] as string)
                }
                treeData={treeData}
                showIcon
              />
              ...
              <Table
                ...
                rowSelection={{
                  ...
                  onChange: (_keys, selectedRows) =>
                    updateSelectionFromCurrentPage(selectedRows),
                  preserveSelectedRowKeys: true,
                  columnWidth: 40,
                  getCheckboxProps: (record: AutoHealing.CMDBItem) => {
                    const excluded = isHostExcluded(record, excludeHosts);
                    return {
                      disabled: excluded,
                      title: excluded ? '该主机已在模板预设中' : undefined,
                    };
                  },
                }}
                pagination={{
                ...
                onRow={(record) => ({
                  ...
                  style: {
                    cursor: isHostExcluded(record, excludeHosts)
                      ? 'not-allowed'
                      : 'pointer',
                  },
                })}
