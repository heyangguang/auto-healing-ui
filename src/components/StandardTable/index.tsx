...
function StandardTable<T extends object>({
  ...
  const { effectiveAdvancedSearchFields, schemaLoadError } =
    useStandardTableSearchSchema(advancedSearchFields, searchSchemaRequest);
  /* ---- 表格状态 ---- */
  ...
  const filterableCols = useMemo(
    () =>
      (columnDefs || []).filter(
        (c) => c.headerFilters && c.headerFilters.length > 0,
      ),
    [columnDefs],
  );
  const {
  ...
  const containerClassName = [
    ...
    fitContentHeight ? 'standard-table-container--fit-content' : '',
  ]
    .filter(Boolean)
    .join(' ');
    ...
    <div className={containerClassName}>
      ...
      {children ? (
        <div className="standard-table-main">
          {schemaLoadError && (
            <StandardTableSchemaErrorAlert error={schemaLoadError} />
          )}
          ...
          {searchFields && searchFields.length > 0 && (
            <StandardTableSearchShell
              ...
              onSearch={() =>
                handleSearch({ pageSize, sorter, fetchData, setPage })
              }
              ...
              showAdvancedToggle={
                !!(
                  ...
                  effectiveAdvancedSearchFields.length > 0
                )
              }
              ...
              onClearFilters={() =>
                handleClearFilters({ pageSize, sorter, fetchData, setPage })
              }
              ...
              onResetAdvanced={() =>
                handleReset({ pageSize, sorter, fetchData, setPage })
              }
        ...
        <div className="standard-table-main">
          ...
          >
            <Table<T>
              ...
              rowKey={rowKey}
              tableLayout={
                Object.keys(columnWidths).length > 0 ? 'fixed' : undefined
              }
              loading={loading}
              pagination={false}
                ...
                onRowClick
                  ? (record) => ({
                      onClick: (e) => {
                        ...
                        if (
                          target.closest(
                            'button, a, input, label, .ant-switch, .ant-btn, .ant-popover, .ant-popconfirm, .ant-checkbox-wrapper, .ant-checkbox, .ant-table-selection-column, svg',
                          )
                        )
                          return;
                        onRowClick(record);
