...
export async function getRoles() {
    return request<{ data: AutoHealing.RoleWithStats[] }>('/api/v1/tenant/roles', {
        method: 'GET',
    });
}
...
export async function getPlatformRoles() {
    return request<{ data: AutoHealing.RoleWithStats[] }>('/api/v1/platform/roles', {
        method: 'GET',
    });
}
...
export async function getRole(id: string) {
    return request<AutoHealing.Role>(`/api/v1/tenant/roles/${id}`, {
        method: 'GET',
    });
}
...
export async function createRole(data: AutoHealing.CreateRoleRequest) {
    return request<AutoHealing.Role>('/api/v1/tenant/roles', {
        method: 'POST',
        data,
    });
}
...
export async function getPlatformRole(id: string) {
    return request<AutoHealing.Role>(`/api/v1/platform/roles/${id}`, {
        method: 'GET',
    });
}
...
export async function createPlatformRole(data: AutoHealing.CreateRoleRequest) {
    return request<AutoHealing.Role>('/api/v1/platform/roles', {
        method: 'POST',
        data,
    });
}
