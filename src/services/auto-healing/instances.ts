...
/** 获取自愈实例列表 GET /api/v1/healing/instances */
...
/** 获取自愈实例详情 GET /api/v1/healing/instances/:id */
export async function getHealingInstanceDetail(id: string, options?: { [key: string]: any }) {
    return request<AutoHealing.FlowInstance>(`/api/v1/healing/instances/${id}`, {
        method: 'GET',
        ...(options || {}),
    });
}

/** 取消自愈实例 POST /api/v1/healing/instances/:id/cancel */
export async function cancelHealingInstance(id: string, options?: { [key: string]: any }) {
    return request<AutoHealing.SuccessResponse>(`/api/v1/healing/instances/${id}/cancel`, {
        method: 'POST',
        ...(options || {}),
    });
}

/** 重试自愈实例 POST /api/v1/healing/instances/:id/retry */
