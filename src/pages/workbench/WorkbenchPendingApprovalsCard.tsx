    ...
    {loading ? (
      ...
      >
        <Empty
          ...
          description="当前卡片仅聚合自愈触发与自愈审批，其他待办请前往待办中心查看"
        />
      </div>
      ...
      <div style={{ padding: 0, flex: 1 }}>
        {pendingApprovals.items.slice(0, 5).map((item) => {
          const severity =
            item._pendingType === 'trigger'
              ? INCIDENT_SEVERITY_MAP[item.severity || 'medium'] ||
                INCIDENT_SEVERITY_MAP.medium
              : null;
          return (
            <div
              key={item.id}
            ...
            >
              <span
                ...
                style={{ background: severity?.color || '#1677ff' }}
              />
              <div className={styles.pendingContent}>
                ...
                >
                  {item.title || item.node_name || '待办任务'}
                </span>
                <Tag
                  color={severity?.color || 'blue'}
                  style={{
                    margin: 0,
                    fontSize: 10,
                    lineHeight: '16px',
                ...
                <span className={styles.pendingType}>
                  {item._pendingType === 'trigger' ? '触发' : '审批'}
                </span>
              </div>
        ...
        {pendingApprovals.total > 5 && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            <Button
              type="link"
              size="small"
              disabled={!canViewPendingCenter}
              onClick={onOpenPendingCenter}
            >
