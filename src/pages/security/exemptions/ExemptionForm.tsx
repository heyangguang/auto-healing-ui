...
const hasFormErrorFields = (
  error: unknown,
): error is { errorFields: unknown } =>
  typeof error === 'object' && error !== null && 'errorFields' in error;

function stopAndRun(event: React.MouseEvent<HTMLElement>, action: () => void) {
...
const ExemptionForm: React.FC = () => {
  ...
  const [selectedTask, setSelectedTask] =
    useState<AutoHealing.ExecutionTask | null>(null);
  const [selectedRule, setSelectedRule] = useState<CommandBlacklistRule | null>(
    null,
  );

  // 任务模板选择回调
  const handleTaskSelect = useCallback(
    (id: string, template: AutoHealing.ExecutionTask) => {
  ...
  const handleRuleSelect = useCallback(
    (id: string, rule: CommandBlacklistRule) => {
  ...
  const clearSelectedRule = useCallback(() => {
    ...
    form.setFieldsValue({ rule_id: undefined });
  }, [form]);
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      message.error(
        extractErrorMsg(
          err as Parameters<typeof extractErrorMsg>[0],
          '提交豁免申请失败，请稍后重试',
        ),
      );
    } finally {
  ...
  return (
    <div className="blacklist-form-page">
      <SubPageHeader
        ...
        actions={
          <div className="blacklist-form-header-actions">
            <Button onClick={() => history.push('/security/exemptions')}>
              取消
            </Button>
            <Button
              type="primary"
      ...
      <Form
        ...
        layout="vertical"
        requiredMark={false}
        initialValues={{ validity_days: 30 }}
        size="large"
      >
        <div className="blacklist-form-cards">
          ...
          <div className="blacklist-form-card">
            ...
            <div
              className={`exemption-selection-box ${selectedTask ? 'selected' : ''}`}
              onClick={() => setTaskSelectorOpen(true)}
            >
              {selectedTask ? (
                <div className="exemption-selection-content">
                  <div className="exemption-selection-main">
                    <Space>
                      <Tag
                    ...
                    <div className="exemption-selection-meta">
                      <Text type="secondary">
                  ...
                  <Button
                    aria-label="取消选择任务模板"
                    className="exemption-selection-clear"
                    icon={<CloseOutlined />}
                    size="small"
                    type="text"
                    onClick={(event) => stopAndRun(event, clearSelectedTask)}
                  />
                </div>
              ) : (
          ...
          <div className="blacklist-form-card">
            ...
            <div
              className={`exemption-selection-box ${selectedRule ? 'selected' : ''}`}
                ...
                <div className="exemption-selection-content">
                  <div className="exemption-selection-main">
                    <Space wrap>
                      <Tag
                        color={
                          getBlacklistSeverityMeta(selectedRule.severity)
                            .tagColor
                        }
                        style={{ margin: 0 }}
                      >
                      ...
                      <Tag style={{ margin: 0, fontSize: 10 }}>
                        {
                          getBlacklistMatchTypeMeta(selectedRule.match_type)
                            .label
                        }
                      </Tag>
                    </Space>
                  ...
                  <Button
                    aria-label="取消选择黑名单规则"
                    className="exemption-selection-clear"
                    icon={<CloseOutlined />}
                    size="small"
                    type="text"
                    onClick={(event) => stopAndRun(event, clearSelectedRule)}
                  />
                </div>
              ) : (
                <Space direction="vertical" size={2} align="center">
