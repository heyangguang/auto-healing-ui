...
import { PAGE_SIZE } from './blacklistRuleFormOptions';
import { findPlaybookById } from './blacklistRuleFormUtils';

const { Text } = Typography;

function treeTitle(title: string, count: number) {
  return (
    <span
      style={{
        display: 'inline-flex',
...
const buildTreeData = (
  ...
  const repoNodes = repositories.map((repo) => {
    const repoPlaybooks = playbookList.filter(
      (playbook) => playbook.repository_id === repo.id,
    );
    return {
      key: `repo-${repo.id}`,
      title: treeTitle(repo.name, repoPlaybooks.length),
  ...
  return [
    {
      key: 'all',
      title: treeTitle('全部任务', tasksTotal),
...
export const useBlacklistTaskSelector = ({
  open,
}: UseBlacklistTaskSelectorOptions) => {
  ...
  const loadBaseData = useCallback(async () => {
    ...
    try {
      ...
      if (repositories.length > 0) {
        setExpandedKeys(
          repositories.slice(0, 3).map((repo) => `repo-${repo.id}`),
        );
      }
    } catch {
      if (baseDataSequenceRef.current.isCurrent(token)) {
  ...
  const displayTasks = useMemo(() => {
    ...
    return tasks.filter(
      (task) => task.playbook_id && repoPlaybookIds.includes(task.playbook_id),
    );
