...
import { Alert, Button, Empty, Form, Input, InputNumber, Popconfirm, Select, Space, Spin, Tag, Typography } from 'antd';
...
const solutionTemplateHeaderIcon = (
  <svg viewBox="0 0 48 48" fill="none">
    ...
    <path d="M31 31l3 3 6-7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
const SolutionTemplatePage: React.FC = () => {
  ...
  return (
    <StandardTable<AutoHealing.IncidentSolutionTemplate>
      ...
      extraToolbarActions={(
        <SortToolbar
          onSortByChange={(value) => setSortBy(value as 'created_at' | 'name' | 'updated_at')}
              ...
              >
                <Space direction="vertical" size={6} style={{ width: '100%' }}>
                  <Space wrap size={[6, 6]}>
                    ...
                    <Tag color="blue">{template.default_close_status || 'resolved'}</Tag>
                    <Tag>{template.steps_render_mode || 'summary'}</Tag>
                  </Space>
              ...
              <div className="solution-templates-editor-header">
                <div style={{ minWidth: 0 }}>
                  ...
                  <Typography.Text type="secondary" ellipsis>
                    {solutionSummary || '按照“问题说明 / 解决方案 / 执行步骤 / 验证结果 / 最终结论”生成标准回写内容'}
                  </Typography.Text>
                </div>
                <Space>
                  ...
                  {!isCreating && selectedTemplate ? (
                    ...
                    >
                      <Button danger icon={<DeleteOutlined />} disabled={!access.canDeletePlugin}>
                        删除
                      </Button>
                    </Popconfirm>
                  ) : null}
                  <Button type="primary" icon={<SaveOutlined />} loading={saving} disabled={!isDirty || !(isCreating ? access.canCreatePlugin : access.canUpdatePlugin)} onClick={() => void handleSave()}>
                    {isCreating ? '创建' : '保存'}
                  </Button>
                </Space>
              </div>
                ...
                {showPreview ? (
                  <div className="solution-templates-preview">
                    {previewSections.sections.map((section) => (
                      <div key={section.key} className="solution-templates-preview-section">
                        <Typography.Title level={5}>{section.title}</Typography.Title>
                        <div className="solution-templates-preview-content">{section.content}</div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div style={{ maxWidth: 980, margin: '0 auto' }}>
                    ...
                    <Form
                      form={form}
                      layout="vertical"
                      initialValues={{ default_close_status: 'resolved', default_close_code: 'auto_healed', steps_render_mode: 'summary', steps_max_count: 6, step_output_max_length: 240 }}
                    ...
                    >
                      ...
                      <Form.Item name="description" label="模板描述">
                        <Input.TextArea rows={2} placeholder="简要描述该模板适用的故障场景和回写目的" />
                      </Form.Item>
                      ...
                      <Space size={16} style={{ display: 'flex' }} align="start">
                        <Form.Item name="steps_render_mode" label="步骤渲染模式" style={{ flex: 1 }}>
                          <Select options={stepsModeOptions} />
                        </Form.Item>
                        <Form.Item name="steps_max_count" label="最多展示步骤数" style={{ flex: 1 }}>
                          <InputNumber min={1} max={20} style={{ width: '100%' }} />
                        </Form.Item>
                        <Form.Item name="step_output_max_length" label="单步输出摘要长度" style={{ flex: 1 }}>
                          <InputNumber min={60} max={1000} step={20} style={{ width: '100%' }} />
                        </Form.Item>
                      </Space>
                      <Form.Item name="problem_template" label="问题说明模板">
                        <Input.TextArea rows={4} placeholder="例如：告警名称：{{ incident.title }}&#10;影响对象：{{ incident.affected_ci }}" />
                      </Form.Item>
                      <Form.Item
                        name="solution_template"
                        label="解决方案模板"
                        rules={[{ required: true, message: '请输入解决方案模板' }]}
                        extra="这里写标准处理方案说明，不是实时执行日志。"
                      >
                        <Input.TextArea rows={5} placeholder="例如：AHS 将按标准服务恢复方案执行：先检查服务状态，再恢复服务并复查健康检查。" />
                      </Form.Item>
                      <Form.Item name="verification_template" label="验证结果模板">
                        <Input.TextArea rows={4} placeholder="例如：执行状态：{{ execution.status }}&#10;执行说明：{{ execution.message }}" />
                      </Form.Item>
                      <Form.Item
                        name="conclusion_template"
                        label="最终结论模板"
                        rules={[{ required: true, message: '请输入最终结论模板' }]}
                      >
                        <Input.TextArea rows={3} placeholder="例如：AHS 已完成自动修复，源工单已自动关闭。" />
                      </Form.Item>
                      <Form.Item name="resolution_template" label="兼容字段：结论模板">
                        <Input.TextArea rows={2} placeholder="可选；未填写时会默认使用最终结论模板" />
                      </Form.Item>
                      <Form.Item name="work_notes_template" label="兼容字段：过程模板">
                        <Input.TextArea rows={3} placeholder="可选；未填写时会默认使用解决方案模板" />
                      </Form.Item>
                    </Form>
                  </div>
                )}
