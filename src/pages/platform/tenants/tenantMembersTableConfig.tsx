...
const INVITATION_STATUS_MAP: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
...
export function createTenantMembersColumns({
  access,
  isLastAdminMember,
  onOpenChangeRole,
}: TenantMembersColumnOptions): TableProps<PlatformTenantMember>['columns'] {
  return [
    {
      title: '成员',
      dataIndex: 'user',
    ...
    },
    {
      ...
      render: (_: unknown, record) => {
        ...
        return (
          <Tooltip title={protectLastAdmin ? '最后一个租户管理员不能被降级' : '变更角色'}>
            <Button
              type="link"
              size="small"
...
export function createTenantInvitationColumns({
  access,
  onCancelInvitation,
  onCopyInvitationLink,
}: TenantInvitationsColumnOptions): TableProps<TenantInvitation>['columns'] {
  return [
    {
      title: '受邀邮箱',
      dataIndex: 'email',
    ...
    },
    {
      ...
      render: (status: string) => {
        const mapped = INVITATION_STATUS_MAP[status] || { label: status, color: 'default', icon: null };
...
export function renderTenantRoleSummary(tenantRoles: AutoHealing.Role[]) {
  if (tenantRoles.length === 0) {
    return <Text type="secondary">暂无角色说明</Text>;
  }

  return tenantRoles.map((role, index) => (
    <span key={role.id}>
