...
export function resolveExecutionRunId(selectedNodeData?: SelectedNodeDataLike | null) {
    const nodeState = selectedNodeData?.state;
    return getStringValue(nodeState?.run?.run_id)
        || getStringValue(nodeState?.run_id)
        || getStringValue(selectedNodeData?.config?.run_id);
}

export function resolveExecutionTaskId(selectedNodeData?: SelectedNodeDataLike | null) {
    const nodeState = selectedNodeData?.state;
    return getStringValue(nodeState?.task_id)
        || getStringValue(nodeState?.run?.task_id)
        || getStringValue(selectedNodeData?.config?.task_id)
        || getStringValue(selectedNodeData?.config?.task_template_id);
}

export function hasExecutionStarted(selectedNodeData?: SelectedNodeDataLike | null) {
    if (selectedNodeData?.type !== 'execution') {
        return false;
    }

    const status = getStringValue(selectedNodeData.state?.status)
