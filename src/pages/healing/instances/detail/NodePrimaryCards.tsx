...
function getNotificationConfigEntries(
  ...
  if (Array.isArray(config.notification_configs)) {
    return config.notification_configs
      ...
      .map((item) => ({
        channelIds: getStringArray(item.channel_ids).concat(
          getStringValue(item.channel_id) ? [String(item.channel_id)] : [],
        ),
        templateId: getStringValue(item.template_id),
      }))
      .filter((item) => item.channelIds.length > 0 || item.templateId);
  }

  const channelIds = getStringArray(config.channel_ids);
  const channelId = getStringValue(
    config.channel_id || config.notification_channel_id,
  );
  if (channelId) {
    channelIds.push(channelId);
  }
  const templateId = getStringValue(
    config.template_id || config.notification_template_id,
  );
...
const NodePrimaryCards: React.FC<NodePrimaryCardsProps> = ({
  ...
  const taskId =
    resolveExecutionTaskId(selectedNodeData) ||
    resolveExecutionTaskTemplateId(selectedNodeData.config);
  const taskTemplateName = resolveExecutionTaskTemplateName(
  ...
  const notificationConfigs = getNotificationConfigEntries(
    selectedNodeData.config,
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Badge
            status={getBadgeStatus(effectiveStatus)}
            text={
              <span style={{ fontWeight: 500 }}>
                {INSTANCE_STATUS_LABELS[effectiveStatus || ''] ||
                  effectiveStatus ||
                  '未执行'}
              </span>
            }
        ...
        >
          ...
          {nodeState?.duration_ms != null && (
            <Descriptions.Item label="执行耗时">
              {nodeState.duration_ms >= 1000
                ? `${(nodeState.duration_ms / 1000).toFixed(1)}s`
                : `${nodeState.duration_ms}ms`}
            </Descriptions.Item>
          )}
          {nodeState?.started_at && (
            <Descriptions.Item label="开始时间">
              {new Date(nodeState.started_at).toLocaleString('zh-CN')}
            </Descriptions.Item>
          )}
          {Boolean(nodeState?.finished_at || nodeState?.updated_at) && (
            <Descriptions.Item label="结束时间">
              {new Date(
                String(nodeState?.finished_at || nodeState?.updated_at),
          ...
          {runId && (
            <Descriptions.Item label="执行记录">
              {resolvedNames[`run:${runId}`] ? (
                <Tag
                  color={
                    resolvedNames[`run:${runId}`] === 'completed'
                      ? 'success'
                      : resolvedNames[`run:${runId}`] === 'failed'
                        ? 'error'
                        : 'processing'
                  }
                  style={{ margin: 0 }}
          ...
          {isExecution && nodeState?.run?.exit_code != null && (
            <Descriptions.Item label="退出码">
              {nodeState.run.exit_code}
            </Descriptions.Item>
          )}
        </Descriptions>
      </Card>
      ...
      {isApproval && nodeState && (
        ...
        >
          ...
          >
            ...
            {nodeState.timeout_at && (
              <Descriptions.Item label="超时期限">
                {new Date(nodeState.timeout_at).toLocaleString('zh-CN')}
              </Descriptions.Item>
            )}
            ...
            {(nodeState.decision_comment || effectiveStatus === 'rejected') && (
              <Descriptions.Item label="决策意见" span={2}>
                <Tag
                  color={effectiveStatus === 'rejected' ? 'error' : 'success'}
                  style={{ fontSize: 13, padding: '2px 10px' }}
                >
                  {nodeState.decision_comment ||
            ...
            {[
              {
                ...
                value:
                  (executionStats.ok || 0) +
                  (executionStats.changed || 0) +
                  (executionStats.unreachable || 0) +
              ...
              {
                label: '成功',
                value: executionStats.ok || 0,
                color: '#52c41a',
              },
              {
                label: '失败',
                value: executionStats.failed || 0,
                color: '#ff4d4f',
              },
              {
                label: '变更',
                value: executionStats.changed || 0,
                color: '#faad14',
              },
              {
                label: '失联',
                value: executionStats.unreachable || 0,
                color: '#ff7a45',
              },
              {
                label: '跳过',
                value: executionStats.skipped || 0,
                color: '#8c8c8c',
              },
            ].map((item) => (
      ...
      {isExecution && stdoutLogs.length > 0 && (
        ...
        >
          ...
          >
            {buildDuplicateSafeEntries(
              stdoutLogs.slice(-30).map((log) => log.message),
            ).map((entry, index) => (
              <div
                key={entry.key}
                style={{
                  color:
                    stdoutLogs.slice(-30)[index].log_level === 'error'
                      ? '#ff6b6b'
                      : stdoutLogs.slice(-30)[index].log_level === 'changed'
                        ? '#faad14'
                        : stdoutLogs.slice(-30)[index].log_level === 'ok'
                          ? '#52c41a'
                          : stdoutLogs.slice(-30)[index].log_level ===
                              'skipping'
                            ? '#8c8c8c'
                            : '#c9d1d9',
                  whiteSpace: 'pre-wrap',
      ...
      {nodeType === 'host_extractor' && nodeState?.extracted_hosts && (
        <Card
          ...
          title={
            <span style={{ fontSize: 13, fontWeight: 600 }}>
              <AimOutlined style={{ color: '#52c41a', marginRight: 6 }} />
              提取结果
            </span>
          }
        ...
        >
          <Space size={[6, 6]} wrap>
            {buildDuplicateSafeEntries(
              Array.isArray(nodeState.extracted_hosts)
                ? nodeState.extracted_hosts
                : String(nodeState.extracted_hosts)
                    .split(',')
                    .map((host) => host.trim())
                    .filter(Boolean),
            ).map((host) => (
              <Tag key={host.key} color="green" style={{ margin: 0 }}>
                {host.value}
              </Tag>
            ))}
          </Space>
          {nodeState.extract_mode && (
            <div style={{ marginTop: 8, fontSize: 12, color: '#8c8c8c' }}>
              提取方式: {nodeState.extract_mode}
              {nodeState.source_field
                ? ` | 来源: ${nodeState.source_field}`
                : ''}
            </div>
          )}
        </Card>
      ...
      {(nodeType === 'condition' || nodeType === 'compute') &&
        nodeState?.activated_branch && (
          <Card
            size="small"
            title={
              <span style={{ fontSize: 13, fontWeight: 600 }}>
          ...
          >
            ...
            >
              <Descriptions.Item label="激活分支">
                <Tag color="purple" style={{ margin: 0, fontSize: 13 }}>
                  {nodeState.activated_branch}
                </Tag>
              </Descriptions.Item>
              {nodeState.matched_expression && (
      ...
      {nodeType === 'set_variable' && nodeState?.variables_set && (
        <Card
          ...
          title={
            <span style={{ fontSize: 13, fontWeight: 600 }}>
              <TagOutlined style={{ color: '#13c2c2', marginRight: 6 }} />
              已设置变量
            </span>
          }
          style={{ marginBottom: 16, borderLeft: '3px solid #13c2c2' }}
        >
          {typeof nodeState.variables_set === 'object' &&
          !Array.isArray(nodeState.variables_set) ? (
            <Descriptions
              column={1}
              size="small"
              bordered
      ...
      {nodeType === 'compute' && nodeState?.computed_results && (
        <Card
          ...
          title={
            <span style={{ fontSize: 13, fontWeight: 600 }}>
              <DashboardOutlined style={{ color: '#fa8c16', marginRight: 6 }} />
              计算结果
            </span>
          }
      ...
      {(nodeType === 'notification' || nodeType === 'send_notification') && (
        <Card
          ...
          title={
            <span style={{ fontSize: 13, fontWeight: 600 }}>
              <BellOutlined style={{ marginRight: 6, color: '#52c41a' }} />
              通知发送
            </span>
          }
          style={{ marginBottom: 16, borderLeft: '3px solid #52c41a' }}
        ...
        >
          <Space direction="vertical" size={10} style={{ width: '100%' }}>
            ...
            >
              <Space size={8} wrap>
                <Tag
                  color={getNotificationStatusColor(notificationStatus)}
                  style={{ margin: 0 }}
                >
                ...
                {nodeState?.sent_at && (
                  <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                    {new Date(nodeState.sent_at).toLocaleString('zh-CN')}
                  </Typography.Text>
                )}
                {nodeState?.duration_ms != null && (
                  <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                    耗时 {nodeState.duration_ms}ms
                  </Typography.Text>
                )}
              </Space>
            </div>
            ...
            {notificationConfigs.length > 0 ? (
              notificationConfigs.map((entry, index) => (
                <div
                  key={`${entry.templateId || 'template'}-${entry.channelIds.join('-')}-${index}`}
                  style={{
                ...
                >
                  <Descriptions
                    column={1}
                    size="small"
                    labelStyle={{ color: '#8c8c8c', width: 72, fontSize: 12 }}
                    contentStyle={{ color: '#262626', fontSize: 13 }}
                  >
                    <Descriptions.Item label="通知模板">
                      {entry.templateId ? (
                        <Tag color="green" style={{ margin: 0 }}>
                          {resolvedNames[entry.templateId] || entry.templateId}
                        </Tag>
                      ) : (
                    ...
                    <Descriptions.Item label="通知通道">
                      {entry.channelIds.length > 0 ? (
                        <Space size={[6, 6]} wrap>
                          {entry.channelIds.map((channelId) => (
                            <Tag
                              key={channelId}
                              color="blue"
                              style={{ margin: 0 }}
                            >
                              {resolvedNames[channelId] || channelId}
                            </Tag>
                          ))}
                        </Space>
                      ) : (
          ...
          {notificationError && (
            <Card bordered={false} bodyStyle={{ padding: 0, marginTop: 10 }}>
