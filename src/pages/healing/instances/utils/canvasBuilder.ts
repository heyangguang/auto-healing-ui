...
function buildCanvasNodes(
    ...
    const nodes: CanvasNode[] = flowNodes.map((node) => {
        ...
        const wasPassedThrough = executedNodeIds.has(node.id)
            && (outgoingEdges[node.id] || []).some((edge) => executedNodeIds.has(edge.target));
        const nodeStatus = nodeState?.status || (wasPassedThrough ? 'success' : undefined);
...
function buildCanvasEdges(
    ...
    return flowEdges.map((edge) => {
        ...
        const isActiveBranch = isEdgeOnActiveBranch(edge, nodeTypeMap, nodeEffectiveStatus);
        const isExecutedEdge = sourceExecuted && targetExecuted && isActiveBranch;
        const isInactiveBranchEdge = isBranchHandle(edge.sourceHandle) && sourceExecuted && !isActiveBranch;
        ...
        return {
            ...
            style: {
                stroke: isExecutedEdge
                    ...
                    : (isInactiveBranchEdge ? getInactiveBranchColor(edge.sourceHandle) : '#d9d9d9'),
...
function injectRuleTrigger(
    ...
    const ruleNode: CanvasNode = {
        id: ruleNodeId,
        type: 'custom',
        position: { x: startNode?.position?.x ?? 0, y: (startNode?.position?.y ?? 0) - 100 },
        data: { label: `自愈规则: ${rule.name}`, type: 'trigger', status: 'triggered', details: rule },
        draggable: false,
        connectable: false,
    };
