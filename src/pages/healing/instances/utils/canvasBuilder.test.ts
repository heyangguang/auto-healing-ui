...
describe('buildCanvasElements', () => {
    ...
    it('高亮带 default sourceHandle 的线性前置路径', () => {
        const { edges, nodes } = buildCanvasElements({
            flowNodes: [
                { id: 'start', type: 'start', name: '开始', position: { x: 0, y: 0 }, config: {} },
                { id: 'extract', type: 'host_extractor', name: '提取工单主机', position: { x: 0, y: 80 }, config: {} },
                { id: 'cmdb', type: 'cmdb_validator', name: 'CMDB验证', position: { x: 0, y: 160 }, config: {} },
                { id: 'approval', type: 'approval', name: '人工审批', position: { x: 0, y: 240 }, config: {} },
                { id: 'exec', type: 'execution', name: '执行磁盘恢复', position: { x: 0, y: 320 }, config: {} },
                { id: 'end', type: 'end', name: '结束', position: { x: 0, y: 400 }, config: {} },
            ],
            flowEdges: [
                { id: 'start-extract', source: 'start', sourceHandle: 'default', target: 'extract' },
                { id: 'extract-cmdb', source: 'extract', sourceHandle: 'default', target: 'cmdb' },
                { id: 'cmdb-approval', source: 'cmdb', sourceHandle: 'default', target: 'approval' },
                { id: 'approval-approved', source: 'approval', sourceHandle: 'approved', target: 'exec' },
                { id: 'approval-rejected', source: 'approval', sourceHandle: 'rejected', target: 'end' },
                { id: 'exec-success', source: 'exec', sourceHandle: 'success', target: 'end' },
            ],
            nodeStates: {
    ...
    it('不会把汇聚到结束节点的未执行通知分支误判为成功', () => {
        const { edges, nodes } = buildCanvasElements({
            flowNodes: [
                { id: 'start', type: 'start', name: '开始', position: { x: 0, y: 0 }, config: {} },
                { id: 'approval', type: 'approval', name: '人工审批', position: { x: 0, y: 80 }, config: {} },
                { id: 'exec', type: 'execution', name: '执行恢复', position: { x: 0, y: 160 }, config: {} },
                { id: 'notify-success', type: 'notification', name: '通知-恢复完成', position: { x: 0, y: 240 }, config: {} },
                { id: 'notify-failed', type: 'notification', name: '通知-恢复失败', position: { x: 160, y: 240 }, config: {} },
                { id: 'notify-rejected', type: 'notification', name: '通知-审批拒绝', position: { x: -160, y: 240 }, config: {} },
                { id: 'end', type: 'end', name: '结束', position: { x: 0, y: 320 }, config: {} },
            ],
            flowEdges: [
                { id: 'start-approval', source: 'start', sourceHandle: 'default', target: 'approval' },
                { id: 'approval-approved', source: 'approval', sourceHandle: 'approved', target: 'exec' },
                { id: 'approval-rejected', source: 'approval', sourceHandle: 'rejected', target: 'notify-rejected' },
                { id: 'exec-success', source: 'exec', sourceHandle: 'success', target: 'notify-success' },
                { id: 'exec-failed', source: 'exec', sourceHandle: 'failed', target: 'notify-failed' },
                { id: 'notify-success-end', source: 'notify-success', sourceHandle: 'default', target: 'end' },
                { id: 'notify-failed-end', source: 'notify-failed', sourceHandle: 'default', target: 'end' },
                { id: 'notify-rejected-end', source: 'notify-rejected', sourceHandle: 'default', target: 'end' },
            ],
            nodeStates: {
                approval: { status: 'approved' },
                exec: { status: 'completed' },
        ...
        expect(edgeMap['notify-failed-end']?.style?.strokeDasharray).toBeUndefined();
        expect(edgeMap['notify-rejected-end']?.animated).toBe(false);
        expect(edgeMap['notify-rejected-end']?.style?.strokeDasharray).toBeUndefined();
    ...
    it('会把旧版 task_id/task_name 归一化到执行节点配置里', () => {
