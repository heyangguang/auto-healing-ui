...
> = {
  execution: {
    ...
    color: NODE_TYPE_COLORS.execution,
  },
  approval: {
    label: '审批',
    icon: <AuditOutlined />,
    color: NODE_TYPE_COLORS.approval,
  },
  condition: {
    label: '条件',
    icon: <ForkOutlined />,
    color: NODE_TYPE_COLORS.condition,
  },
  notification: {
    label: '通知',
    icon: <BellOutlined />,
    color: NODE_TYPE_COLORS.notification,
  },
  host_extractor: {
    label: '主机提取',
    icon: <ApiOutlined />,
    color: NODE_TYPE_COLORS.host_extractor,
  },
  cmdb_validator: {
    label: 'CMDB验证',
    icon: <EyeOutlined />,
    color: NODE_TYPE_COLORS.cmdb_validator,
  },
  compute: {
    label: '计算',
    icon: <NodeIndexOutlined />,
    color: NODE_TYPE_COLORS.compute,
  },
};
import './RuleForm.css';
...
const RuleFormPage: React.FC = () => {
  ...
  const [selectedFlow, setSelectedFlow] =
    useState<AutoHealing.HealingFlow | null>(null);
  ...
  const loadFlowById = useCallback(async (flowId: string) => {
    try {
      const flows = await fetchAllPages<AutoHealing.HealingFlow>(
        (page, pageSize) => getFlows({ page, page_size: pageSize }),
      );
  ...
  const loadRule = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setConditions(normalizeRuleConditionsForForm(rule.conditions || []));
      } catch {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload = {
        ...
        conditions: normalizeRuleConditionsForPayload(conditions || []),
      };
      if (isEdit && params.id) {
    ...
    <div className="rule-form-page">
      <SubPageHeader
        ...
        actions={
          <Space>
            ...
            <Button onClick={() => history.push('/healing/rules')}>取消</Button>
            <Button
              type="primary"
              icon={<SaveOutlined />}
              loading={saving}
      ...
      <div className="rule-form-cards">
        ...
        <div className="rule-form-card">
          <h4 className="rule-form-section-title">
            <SafetyCertificateOutlined />
            基础信息
          </h4>
          <Form
