...
type FlowValidationResult =
  | {
      deadEndNodeLabels: string[];
      ok: false;
      reason: 'dead-end' | 'unreachable-branch';
    }
  | { ok: true };
...
function canReachEnd(
  ...
  const result = targets.every((targetId) =>
    canReachEnd(adjacencyMap, targetId, endNodeIds, cache, new Set(path)),
  );
...
export async function validateExecutionNodes(
  ...
  for (const node of executionNodes) {
    ...
    if (!taskTemplateId) {
      return {
        issue: { missingVars: ['task_template_id'], node },
    ...
    const requiredVars = (playbook?.variables || []).filter(
    ...
    const extraVars =
      (node.data?.extra_vars as Record<string, unknown> | undefined) || {};
    const mappings =
      (node.data?.variable_mappings as Record<string, string> | undefined) ||
      {};
