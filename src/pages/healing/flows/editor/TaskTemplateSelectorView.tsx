...
type TaskTemplateSelectorViewProps = ReturnType<
  typeof useTaskTemplateSelectorState
>;
const PANEL_HEIGHT = 420;

function TreeTitle({ count, title }: { count: number; title: string }) {
  return (
    <span
      style={{
        display: 'inline-flex',
...
function TaskTemplateTreePanel({
  ...
  const treeData = useMemo((): DataNode[] => {
    const repoNodes = buildTaskTreeBranches(
      repositories,
      playbooks,
      treeTasks,
    ).map((branch) => ({
      key: `repo-${branch.repoId}`,
      title: <TreeTitle title={branch.repoName} count={branch.taskCount} />,
      icon: <FolderOutlined style={{ color: '#1890ff' }} />,
      children: branch.playbooks.map((playbook) => ({
        key: `playbook-${playbook.playbookId}`,
        title: (
          <TreeTitle title={playbook.playbookName} count={playbook.taskCount} />
        ),
    ...
    return [
      {
        key: 'all',
        title: <TreeTitle title="全部任务" count={treeTasks.length} />,
...
> & { task: TaskTemplate }) {
  const playbookKnownOffline =
    !!task.playbook &&
    !['ready', 'outdated'].includes(String(task.playbook.status));
...
function TaskTemplateSelectedBanner({
  selectedTask,
}: TaskTemplateSelectorViewProps) {
  if (!selectedTask) {
    return null;
...
const TaskTemplateSelectorView: React.FC<TaskTemplateSelectorViewProps> = (
  props,
) => {
  return (
    <>
