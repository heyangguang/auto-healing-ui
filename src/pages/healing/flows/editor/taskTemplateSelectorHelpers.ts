...
export function getPlaybookIdFromTreeKey(treeKey: string) {
  return treeKey.startsWith('playbook-')
    ? treeKey.replace('playbook-', '')
    : undefined;
}

export function getRepoPlaybookIds(
...
const SELECTABLE_PLAYBOOK_STATUSES = new Set(['ready', 'outdated']);

export function isTaskSelectable(task: TaskTemplate) {
  return (
    !task.needs_review &&
    (!task.playbook ||
      SELECTABLE_PLAYBOOK_STATUSES.has(String(task.playbook.status)))
  );
}
...
export function getTreeInventoryTasks(
  ...
  const keepSelectedTaskVisible =
    !filters.search && !filters.executorType && !filters.statusFilter;
  return dedupeTasks(
    tasks.filter((task) => {
...
export function buildTaskTreeBranches(
  ...
  const taskCountByPlaybookId = tasks.reduce<Map<string, number>>(
    (counts, task) => {
      const playbookId = getTaskPlaybookId(task);
      if (!playbookId) {
        return counts;
  ...
  return repositories.reduce<TreeBranch[]>((branches, repository) => {
    ...
    branches.push({
      ...
      taskCount: repoPlaybooks.reduce(
        (count, playbook) => count + playbook.taskCount,
        0,
      ),
    });
    return branches;
