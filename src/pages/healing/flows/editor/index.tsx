...
const FlowEditorInner: React.FC = () => {
  ...
  const [closePolicy, setClosePolicy] = useState<
    AutoHealing.FlowClosePolicy | undefined
  >();
  ...
  const fetchFlow = useCallback(
    async (flowId: string) => {
      ...
      try {
        ...
        const mapped = mapFlowResponseToGraph(response.data, () =>
          setDryRunOpen(true),
        );
        setFlowName(mapped.flowName);
        setFlowIsActive(mapped.flowIsActive);
        setAutoCloseEnabled(mapped.autoCloseEnabled);
  ...
  useEffect(() => {
    ...
    setAutoCloseEnabled(initialState.autoCloseEnabled);
  ...
  const onConnect = useCallback(
    (params: Connection) =>
      setEdges((currentEdges) =>
        addEdge(createConnectedEdge(params), currentEdges),
      ),
    [setEdges],
  );
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      setNodes((currentNodes) =>
        currentNodes.concat(
          createDroppedNode(type, label, position, () => setDryRunOpen(true)),
        ),
      );
    },
  ...
  const onNodeContextMenu = useCallback(
    (event: React.MouseEvent, node: FlowEditorNode) => {
      ...
      setMenu(
        buildContextMenuState(event, reactFlowWrapper.current, node.id, 'node'),
      );
    },
    [nodes],
  );
  ...
  const onEdgeContextMenu = useCallback(
    (event: React.MouseEvent, edge: FlowEditorEdge) => {
      event.preventDefault();
      setMenu(
        buildContextMenuState(event, reactFlowWrapper.current, edge.id, 'edge'),
      );
    },
    [],
  );
  ...
  const onMenuClick = useCallback(() => {
    ...
    if (menu.type === 'edge') {
      setEdges((currentEdges) =>
        currentEdges.filter((edge) => edge.id !== menu.id),
      );
    ...
    setNodes((currentNodes) =>
      currentNodes.filter((node) => node.id !== menu.id),
    );
    setEdges((currentEdges) =>
      currentEdges.filter(
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!structureValidation.ok) {
      if (structureValidation.reason === 'dead-end') {
        message.error(
          `节点 "${structureValidation.deadEndNodeLabels.join('", "')}" 没有连接到结束节点`,
        );
      } else {
    ...
    if (executionValidation.issue) {
      const nodeName = String(
        executionValidation.issue.node.data?.label || '任务执行',
      );
      if (executionValidation.issue.missingVars[0] === 'task_template_id') {
        message.error(`节点 "${nodeName}" 未选择作业模板`);
      } else {
        message.error(
          `节点 "${nodeName}" 的必填变量未填写: ${executionValidation.issue.missingVars.join(', ')}`,
        );
      }
    ...
    if (executionValidation.unavailableNodeLabels.length > 0) {
      message.warning(
        `节点 "${executionValidation.unavailableNodeLabels.join('", "')}" 的远程模板校验暂不可用，已继续保存`,
      );
    }

    if (autoCloseEnabled && !closePolicy?.solution_template_id) {
  ...
  }, [autoCloseEnabled, closePolicy, edges, flowIsActive, flowName, id, nodes]);

  const handleLayout = useCallback(() => {
    const layoutedNodes = applyAutoLayout(nodes, edges);
    setNodes(layoutedNodes);
    setEdges((currentEdges) =>
      currentEdges.map((edge) => ({ ...edge, type: 'smoothstep' })),
    );
        ...
        >
          <FlowEditorHeader
            ...
            hasConfiguredCloseTemplate={Boolean(
              closePolicy?.solution_template_id,
            )}
          ...
          <FlowEditorWorkspace
            ...
            onInit={(instance) =>
              setReactFlowInstance(instance as typeof reactFlowInstance)
            }
