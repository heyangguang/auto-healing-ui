...
import {
  ...
  hasEffectiveNotificationConfig,
...
const drawerJsonPanelStyle: React.CSSProperties = {
  width: '100%',
  maxWidth: '100%',
    ...
    <Drawer
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span>
            {drawerType === 'task' ? '任务模板详情' : 'Playbook 详情'}
          </span>
        </div>
      }
      extra={
      ...
      {drawerType === 'task' && task && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="industrial-dashed-box">
            ...
            <Field label="执行器">
              <Tag
                ...
                color={executorConfig.tagColor || executorConfig.color}
              ...
              {task.secrets_source_ids?.length ? (
                <Space size={4} wrap>
                  {task.secrets_source_ids.map((sourceId) => {
                    const source = secretsSources.find(
          ...
          <div className="industrial-dashed-box">
            <SectionTitle
              icon={<BellOutlined />}
              title="通知配置 / NOTIFICATION"
            />
            <Field label="通知状态">
              {hasEffectiveNotificationConfig(
                task.notification_config as never,
              ) ? (
                <Tag
                  icon={<BellOutlined />}
                  color="green"
            ...
            {getEnabledNotificationTriggers(task.notification_config as never)
              .length > 0 && (
              <Field label="触发条件">
                <Space size={4} wrap>
                  {getEnabledNotificationTriggers(
          ...
          {task.extra_vars && Object.keys(task.extra_vars).length > 0 && (
            <div className="industrial-dashed-box">
              <SectionTitle
                icon={<CodeOutlined />}
                title="额外变量 / EXTRA VARS"
              />
              <pre style={drawerJsonPanelStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="industrial-dashed-box">
            ...
            <Field label="配置模式">
              <Tag
                style={{ margin: 0, fontSize: 11 }}
                color={playbook.config_mode === 'enhanced' ? 'blue' : 'default'}
              >
            ...
            <div className="industrial-dashed-box">
              ...
              <Field label="同步状态">
                <Tag
                  style={{ margin: 0, fontSize: 11 }}
                  color={
                    playbook.repository.status === 'ready' ? 'green' : 'orange'
                  }
                >
          ...
          {playbook.variables?.length ? (
            <div className="industrial-dashed-box">
              <SectionTitle
                icon={<CodeOutlined />}
                title={`变量定义 / VARIABLES (${playbook.variables.length})`}
              />
                ...
                {playbook.variables.map((variable) => (
                  ...
                  >
                    ...
                    {variable.description && (
                      <div
                        style={{ color: '#8c8c8c', fontSize: 11, marginTop: 2 }}
                      >
                        {variable.description}
                      </div>
                    )}
          ...
          {playbook.last_scanned_at && (
            ...
            >
              最后扫描于{' '}
              {dayjs(playbook.last_scanned_at).format('YYYY-MM-DD HH:mm:ss')}
            </div>
          )}
        </div>
      )}
