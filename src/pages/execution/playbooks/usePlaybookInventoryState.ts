...
type PlaybookDateRangeValue = [
  Parameters<typeof toDayRangeStartISO>[0],
  Parameters<typeof toDayRangeEndISO>[0],
];
type PlaybookAdvancedSearchParams = Record<string, unknown> & {
  created_at?: PlaybookDateRangeValue;
};

function mapPlaybookStats(
  statsResponse: Awaited<ReturnType<typeof getPlaybookStats>>,
) {
  const byStatus = statsResponse.by_status || [];
  const getCount = (status: string) =>
    byStatus.find((item) => item.status === status)?.count || 0;
  return {
...
function getPlaybookStatsKey(
  status?: AutoHealing.PlaybookStatus,
): keyof PlaybookStatsState | undefined {
...
function decrementPlaybookStats(
...
function useExpandedRepoKeys(
  groupedPlaybooks: Record<
    string,
    {
      repo: AutoHealing.GitRepository | null;
...
export function usePlaybookInventoryState() {
  ...
  const loadPlaybooks = useCallback(
    async (params?: PlaybookSearchParams) => {
      ...
      try {
        const items = await fetchAllPages<AutoHealing.Playbook>(
          (page, pageSize) =>
            getPlaybooks({ ...queryParams, page, page_size: pageSize }),
        );
        ...
        if (playbookRequestIdRef.current === requestId) {
          const messageText =
            error instanceof Error ? error.message : '加载 Playbook 列表失败';
      ...
      try {
        ...
        if (playbookRequestIdRef.current === requestId) {
          const messageText =
            error instanceof Error ? error.message : '加载 Playbook 统计失败';
          message.error(messageText);
        }
      }
    },
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      .then(([playbookResult, repoResult, statsResult]) => {
        if (
          playbookResult.status === 'fulfilled' &&
          playbookRequestIdRef.current === requestId
        ) {
          commitPlaybooks(playbookResult.value);
        } else if (
          playbookResult.status === 'rejected' &&
          playbookRequestIdRef.current === requestId
        ) {
  ...
  const mergePlaybookInInventory = useCallback(
    (playbook: AutoHealing.Playbook) => {
      const currentItems = playbooksRef.current;
      const currentPlaybook = currentItems.find(
      ...
      commitPlaybooks(
        currentItems.map((item) =>
          item.id === playbook.id ? { ...item, ...playbook } : item,
        ),
      );
      setStats((currentStats) =>
        transitionPlaybookStats(
          currentStats,
  ...
  const removePlaybookFromInventory = useCallback(
    (playbook: AutoHealing.Playbook) => {
      commitPlaybooks(
        playbooksRef.current.filter((item) => item.id !== playbook.id),
      );
      setStats((currentStats) =>
        decrementPlaybookStats(currentStats, playbook.status),
      );
    },
    [commitPlaybooks],
  );
  ...
  const groupedPlaybooks = useMemo(() => {
    ...
    playbooks.forEach((playbook) => {
      if (!groups[playbook.repository_id]) {
        groups[playbook.repository_id] = {
          playbooks: [],
          repo:
            repos.find((repo) => repo.id === playbook.repository_id) ||
            playbook.repository ||
            null,
        };
      }
  ...
  const handleSearch = useCallback(
    ({
      ...
      if (Array.isArray(createdAt) && createdAt.length === 2) {
        if (createdAt[0])
          params.created_from = toDayRangeStartISO(createdAt[0]);
