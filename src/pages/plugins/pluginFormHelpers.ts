...
export const BASIC_AUTH_USERNAME_RULES: Rule[] = [
  { required: true, whitespace: true, message: '请输入用户名' },
];

export type PluginFilterOperator = Extract<
  AutoHealing.SyncFilterRule['operator'],
  string
>;
...
export function buildPluginConfig(
  ...
  if (values.auth_type === 'basic') {
    ...
    else if (originalConfig?.auth_type === 'basic' && originalConfig.password)
      config.password = originalConfig.password;
  }
  if (values.auth_type === 'bearer') {
    if (values.token) config.token = values.token;
    else if (originalConfig?.auth_type === 'bearer' && originalConfig.token)
  ...
  if (values.auth_type === 'api_key') {
    ...
    else if (originalConfig?.auth_type === 'api_key' && originalConfig.api_key)
      config.api_key = originalConfig.api_key;
  }
  if (values.since_param) config.since_param = values.since_param;
  if (values.response_data_path)
    config.response_data_path = values.response_data_path;
  if (values.close_incident_url)
    config.close_incident_url = values.close_incident_url;
  if (values.close_incident_method)
    config.close_incident_method = values.close_incident_method;
  return config;
}

export function buildPluginExtraParams(
  extraParams: EditablePluginExtraParam[],
...
export function buildPluginFieldMapping(
  ...
  const validMappings = mappings.filter(
    (mapping) => mapping.standard && mapping.external,
  );
  ...
  return type === 'cmdb'
    ? { cmdb_mapping: mappingRecord }
    : { incident_mapping: mappingRecord };
}

export function buildPluginSyncFilter(
  filters: EditablePluginFilter[],
): AutoHealing.SyncFilter {
  const rules = filters
...
export function hasIncompletePluginFilter(
  filters: EditablePluginFilter[],
): boolean {
...
export function buildPluginPayload({
  ...
  return {
    ...
    sync_interval_minutes:
      values.sync_interval_minutes ?? DEFAULT_PLUGIN_SYNC_INTERVAL_MINUTES,
    max_failures: values.max_failures ?? DEFAULT_PLUGIN_MAX_FAILURES,
  };
}

export function getPluginEditInitialValues(
  plugin: PluginRecord,
): Partial<PluginFormValues> {
  return {
    name: plugin.name,
    type: plugin.type,
    ...
    sync_interval_minutes:
      plugin.sync_interval_minutes ?? DEFAULT_PLUGIN_SYNC_INTERVAL_MINUTES,
